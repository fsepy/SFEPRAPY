...
def wall_HT_sub(
    gas_temp, temp_arr, nodes, dt, dx, hchot, hccold, effemish, effemisc, tamb
):
...
if __name__ == "__main__":
    ...
    wall_depth_arr = np.ones(
        nodes
    )  # creates an array for interpolating an isotherm position
    ...
    f = (
        np.ones(nodes) * iso
    )  # Creates a straight line with same number of indices for isotherm tracking
    ...
    for tstep in range(1, endtimestep + 1):
        ...
        temp_arr = wall_HT_sub(
            Tgas, temp_arr, nodes, dt, dx, hchot, hccold, emissh, emissc, Tambient
        )
        Twall = temp_arr[0]
